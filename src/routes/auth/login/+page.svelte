<script lang="ts">
  ...
  onMount(() => {
    // Check if user has social session
...
<div class="min-h-screen flex items-center justify-center bg-surface-300 p-4">
  <div class="w-full max-w-md backdrop-blur-md bg-white/20 border border-white/30 shadow-lg rounded-xl p-8">
    <div class="flex justify-center mb-8">
      <img src="/logo-vertical.svg" alt="Treasury" class="h-48" />
    </div>
    ...
    <button 
      ...
      disabled={loading}
      class="border border-gray-400 text-gray-700 hover:bg-gray-50 inline-flex items-center justify-center font-medium transition-all duration-200 focus:outline-none px-6 py-3 text-sm rounded-xl w-full justify-center space-x-2 disabled:opacity-50"
    >
      {#if loading}
